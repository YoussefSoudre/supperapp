...
let AdminService = class AdminService {
    ...
    permRepo;
    userRoleRepo;
    constructor(roleRepo, permRepo, userRoleRepo) {
    ...
    async assignRole(userId, roleId, grantedBy, cityId) {
        const existing = await this.userRoleRepo.findOne({ where: { userId, roleId } });
        if (existing)
            return existing;
        return this.userRoleRepo.save(this.userRoleRepo.create({ userId, roleId, grantedBy, cityId: cityId ?? null, expiresAt: null }));
    }
